...
- Comparison Lab / Difference Finder V1: `archeoqa/backend/services/difference_service.py` compares 2-5 selected papers from manifest/Matrix only, then can hand off to Chat with the real `paper_filter` preselected for cited Compare. It must not call PaperQA retrieval or an LLM.
- Gap Finder V1: `archeoqa/backend/services/gap_service.py` reports Matrix coverage gaps from manifest/Matrix only. It separates review-only rows from true completion gaps and must not call PaperQA retrieval or an LLM.
- Contradiction Detector V1: `archeoqa/backend/services/contradiction_service.py` reports Matrix-only candidate tensions, not confirmed contradictions. It must not call PaperQA retrieval or an LLM; final scientific validation should go through cited Compare.
...
- Known Comparison Lab polish item: split broad shared values into `central shared` vs `contextual shared`. Current `Commun` can show broad/contextual overlaps such as Canary Islands, Iberia, Levant, Neolithic, or mtDNA; these are intentionally no longer injected into the suggested cited-Compare question, but the UI could label them more clearly later.
- Known Contradiction Detector polish item: suggested cited-Compare questions can still inject too many shared axes from broad syntheses or indirect site overlaps. Final pass should keep the question focused on the strongest central tension only, while leaving broader shared axes visible for inspection.
- Known Matrix confidence polish item: do not hide or upgrade low-confidence values, but avoid misleading field-level labels. A Matrix cell can show strong visible values while one hidden `+N` value is low; future UI should summarize confidence distribution (for example `2 high · 10 medium · 1 low` or `contains 1 low`) and keep exact per-item badges in the detail view.
